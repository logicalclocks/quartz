...
function A(o) {
  ...
  if (x) {
    ...
    d = d.concat(["nonlocal", "False", "True", "None", "async", "await"]), a = a.concat(["ascii", "bytes", "exec", "print"]);
  ...
  function v(n, e, r) {
    ...
    for (var u = 0; u < h.length; u++)
      if (n.match(h[u]))
        return "operator";
    return n.match(O) ? "punctuation" : e.lastToken == "." && n.match(y) ? "property" : n.match(I) || n.match(U) ? "keyword" : n.match(L) ? "builtin" : n.match(/^(self|cls)\b/) ? "self" : n.match(y) ? e.lastToken == "def" || e.lastToken == "class" ? "def" : "variable" : (n.next(), r ? null : p);
  }
  function R(n, e) {
  ...
  return {
    ...
    languageData: {
      autocomplete: F.concat(E).concat(["exec", "print"]),
...
const Z = A({}), P = A({
  extra_keywords: m("by cdef cimport cpdef ctypedef enum except extern gil include nogil property public readonly struct union DEF IF ELIF ELSE")
});
export {
  P as cython,
  A as mkPython,
